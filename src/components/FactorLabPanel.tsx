...
export function FactorLabPanel({ chain, spot, ticker, live }: Props) {
  ...
  const factors = useMemo(() => {
    const samples = priceBuffer.length;
    if (samples < MIN_SAMPLES) return { samples, ready: false as const };
    const series = BASKET.map(({ t }) => logReturns(priceBuffer.map((s) => s[t])));
    ...
    return { samples, ready: true as const, corr, pca, avgAbs };
  }, [liveSpotPrices]); // recompute each live tick
  ...
  const state = !factors.ready ? 'required' : live ? 'live' : 'model';
  ...
  const stateLabel = state === 'live' ? 'Live Chain' : state === 'model' ? 'Model Mode' : 'Data Required';
  ...
  return (
    <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] overflow-hidden">
      <div className="flex items-center justify-between px-3.5 py-2 border-b border-[var(--border)]">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[8px] uppercase tracking-widest text-[var(--text-tertiary)] tabular-nums">{factors.samples}/{MAX_SAMPLES} samples</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 p-3">
        ...
        <div className="rounded-md border border-[var(--border)] bg-[var(--surface-2)] p-3">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[9px] uppercase tracking-wider text-[var(--text-tertiary)] flex items-center gap-1.5"><Layers className="w-3 h-3" /> Cross-Asset Correlation (log-returns)</span>
            {factors.ready && <span className="text-[9px] tabular-nums text-[var(--text-tertiary)]">⟨|ρ|⟩ {factors.avgAbs.toFixed(2)}</span>}
          </div>
          {factors.ready ? (
        ...
        <div className="rounded-md border border-[var(--border)] bg-[var(--surface-2)] p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            {factors.ready && <span className="text-[9px] tabular-nums text-[var(--text-tertiary)]">PC1 {(factors.pca!.explained[0] * 100).toFixed(0)}% · PC2 {(factors.pca!.explained[1] * 100).toFixed(0)}%</span>}
          </div>
          {factors.ready ? <PcaScatter pca={factors.pca!} /> : <Warming samples={factors.samples} min={MIN_SAMPLES} />}
...
function PcaScatter({ pca }: { pca: ReturnType<typeof pcaFromCorrelation> }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto block" style={{ maxHeight: 170 }}>
      ...
      {pca.coords.map((c, i) => (
        <g key={i}>
          <circle cx={sx(c.pc1)} cy={sy(c.pc2)} r={4} fill={ROLE_CSS[BASKET[i].role]} opacity={0.85} />
          <text x={sx(c.pc1) + 5} y={sy(c.pc2) + 3} fontSize={7} fill="var(--text-secondary)" className="tabular-nums">{BASKET[i].t}</text>
        </g>
      ))}
    </svg>
  );
