...
export function EmptyStatePanel({ icon, title, description, action, className = '', compact }: Props) {
  return (
    <div className={`flex flex-col items-center justify-center gap-2 rounded-[10px] border border-[var(--border-subtle)] bg-[var(--surface)] text-center ${compact ? 'py-8 px-4' : 'py-12 px-6'} ${className}`}>
      ...
      {description && (
        <p className="max-w-sm text-[12px] leading-relaxed text-[var(--text-tertiary)]">{description}</p>
      )}
      {action && (
        <button
          onClick={action.onClick}
          className="mt-2 inline-flex items-center gap-1.5 rounded-[7px] border border-[var(--border)] bg-[var(--surface-2)] px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)] transition-colors hover:border-[var(--border-strong)] hover:text-[var(--text-primary)] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--ring)]"
        >
