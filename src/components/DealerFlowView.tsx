...
export function DealerFlowView({ forcedView, showToggle = true }: { forcedView?: 'profile' | 'targets'; showToggle?: boolean } = {}) {
  ...
  const expiryStatus = isMultiExpiry
    ...
    : expiryTab === 'aggregated'
      ...
      : (() => {
          ...
          return t ? `${t.date} · ${t.dteDays}DTE` : 'select expiry';
        })();
  ...
  return (
    <div className="w-full space-y-[var(--gap)]" id="dealerflow-main-workspace-view">
      ...
      <div className="grid grid-cols-1 items-start gap-[var(--gap)] xl:grid-cols-12">
        <TerminalPanel className="xl:col-span-4" title="Order Flow" subtitle="cumulative tape delta · regime">
      ...
      <div className="grid grid-cols-1 items-start gap-[var(--gap)] xl:grid-cols-12">
        <TerminalPanel className="xl:col-span-7" title="Real-Time Flow" subtitle="multi-ticker · spots" padded={false}>
          <DataTable
